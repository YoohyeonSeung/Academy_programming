...
###############################################
## Chapter17_시계열분석(TimeseriesAnalysis)
###############################################

# 시계열(Time-Series) : 관측치 또는 통계량의 변화를 시간의 움직임에 따라서 기록하고, 이것을 계열화한 것을 의미한다.
# 시계열 데이터 : 통계숫자를 시간의 흐름에 따라 일정한 간격마다 기록한 통계열을 의미한다.
...
# 단계1: WWWusage 데이터셋 가져오기 - R에서 기본 제공 데이터 셋으로 인터넷 사용 시간을 분 단위로 측정한 100개 vector로 구성된 시계열 자료.
data("WWWusage")
...
plot(tsdata - m$seasonal - m$trend) #불규칙 요인말 출력.
