...
# - 자료분석에 필요한 데이터를 대상으로 불필요한 데이터를 처리하는 필터링과 
#   전처리 방법에 대해서 알아본다.
...
attributes(dataset) # 컬럼명/ 자료구조 정보/ 행이름
str(dataset) # 데이터 구조보기(자료구조/관측치(행)/컬럼명(열)/자료형)

# 1.3 데이터 셋 조회
dataset$age
dataset$resident
length(dataset$age) # data 수(행) : 300개
...
dataset[,3] # 3번째 변수(컬럼/열) 전체-출력형태:행 중심 
...
dataset # gender 변수 데이터 정제 
length(dataset$gender) # 300 - 297 = 3개 정제됨.
pie(table(dataset$gender))
pie(table(dataset$gender), col=c('red', 'blue'))

# 3.2 연속형 변수의 극단치 처리.
dataset <- read.csv('dataset.csv', header = T)
dataset$price # 세부테이터 보기 
...
dataset2$survey <- csurvey # survey 수정
...
# 연속형(age) vs 범주형(job2) 데이터 분포 시각화 
...
# 연속형(price) vs 범주형(gender) vs 범주형(position) 데이터 분포 시각화

# (1) 성별에 따른 직급별 구매비용 분포 현황 분석 
densityplot( ~ price | factor(gender2), data = new_data, groups = position2,
             plot.points=T, auto.key = T)


# (2) 직급에 따른 성별 구매비용 분석
densityplot( ~ price | factor(position2), data = new_data, groups = gender2,
             plot.points=T, auto.key = T)
...
# 고객 식별번호(user_id) 대 상품유형(product_type)간의 1:1 파생변수 생성
...
head(product_price, 3) # 컬럼명 수정 확인


# 고객식별번호(user_id) 대 고객지불유형(pay_method)간의 1:1 파생변수 생성
...
# 병합(위에 결과)를 대상으로 고객별 지불 유형에 따른 구매상품 개수 병합하기 
...
choice3 <- sample(50:100, 30) # sample(c(50:100), 30)
choice3
