...
shapiro.test(x1)
# 정규분포 검정 함수(p-value = 0.7242), 표본이 정규 분포로부터 추출된 것인지 테스트하기 위한 방법. 이때 귀무가설은 주어진 데이터가 정규 분포로부터의 표본이라는 것이다.
...
b1 <- b$score # 실시간 coding 교육 받은 학생 점수
...
# p-value = 0.0411 < 0.05 - 교육방법에따른두집단간실기시험의평균에차이가있다.
...
#   - 일반적으로 사전검사와 사후검사의 평균차이를 검증할 때 많이 사용.
