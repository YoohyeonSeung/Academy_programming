...
#   - 탐색기를 통해서 파일 선택하기
# C:/week/R/data/student1.txt 파일 선택
...
html_cont <- as.data.frame(html_cont) # data.frame형변환
str(html_cont) 
head(html_cont)
...
getwd()
setwd("C:/workspaces/Rwork/output") # 작업디렉토리 변경 
library(RSADBE)
...
# 단계3 : 'row.names=F' 속성을 이용하여 행 이름 제거하여 저장한다.
...
GDP_ranking16    # 데이터 프레임 확인
getwd()
setwd("C:/workspaces/Rwork/output") # 저장할 작업 디렉토리 지정 
...
# C:/week/R/data/studentexcel.xlsx 파일 선택 
...
st.df
write.csv(st.df, "stdf.csv", row.names = F, quote = F) # 행 이름 / 따옴표 제거 
